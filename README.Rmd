...
This branch is used for simulation purpose. The difference between this branch and the main branch is that in this simulation branch, if there is only one algorithm used in SuperLearner, then the algorithm is forced to get weight 1 whereas by default, the algorithm will get a weight of either 1 or 0. We forced the weight to be 1 such that in simulation-4 of the paper, the prediction is fully from the random forest algorithm.
