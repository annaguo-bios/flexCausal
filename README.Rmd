---
title: "Causal Effect Estimation in Graphical Models with Unmeasured Variables"
output: 
  github_document:
    toc: true
...
![](vignettes/pics/background.png){width=100%}
...
- `is.fix`: return whether a treatment variable is fixable in a graph object. For example, to check whether the treatment variable `A` is fixable in the graph object for the ADMG in Figure (a), we can use the following code:

```{r, is-fix, warning=F, message=F, eval=F}
is.fix(graph_a, 'A')
```
